...
int main() {
  ...
  // Create a kd-tree for M, note that M must stay valid during the lifetime of
  // the kd-tree.
  ...
  // Look for the 3 nearest neighbours of each query point, use the data
  // themselves for the query. We do not want approximations but we want to sort
  // by the distance, and we want to allow self-match.
  indices.resize(3, M.cols());
  dists2.resize(3, M.cols());
  nns->knn(M, indices, dists2, 3, 0,
           NNSearchF::SORT_RESULTS | NNSearchF::ALLOW_SELF_MATCH);
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    if (indices.coeff(0, i) != i)
      cerr << "Oups, something went wrong: " << indices.coeff(0, i) << " " << i
           << endl;
  }
